  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button asChild variant="accent" size="default" className="gap-2 font-semibold shadow-[var(--shadow-soft)]">
          <Link href="/trips/new">
      ...
      {/* Error Banner */}
      {error && (
        <div className="flex items-center justify-between rounded-2xl border border-destructive/30 bg-destructive/10 p-4 text-destructive shadow-sm">
      ...
      {isLoading && (
        <div className="grid gap-6 sm:grid-cols-2">
          {[1, 2].map((i) => (
            <GlassCard key={i} className="p-6 space-y-4 animate-pulse">
      ...
      {!isLoading && !error && trips.length === 0 && (
        <GlassCard className="p-12 text-center space-y-5 max-w-xl mx-auto my-8">
          <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-ocean/10 text-ocean">
          ...
          <div className="pt-2">
            <Button asChild variant="accent" size="lg" className="gap-2 font-semibold shadow-[var(--shadow-soft)]">
              <Link href="/trips/new">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {trips.map((trip) => {
            ...
            return (
              <GlassCard
                key={trip.id}
                className="group relative flex flex-col justify-between p-6 transition-all duration-200 hover:shadow-[var(--shadow-glass)] hover:border-mist/40"
              >
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <button
                      ...
                      disabled={isDeleting}
                      title="Delete trip"
                      className="rounded-lg p-1.5 text-slate transition-colors hover:bg-destructive/10 hover:text-destructive focus:outline-none focus:ring-2 focus:ring-destructive/30"
                    >
                      {isDeleting ? (
                  ...
                  <div className="space-y-1">
                    <h2 className="text-xl font-bold tracking-tight text-foreground transition-colors group-hover:text-ocean">
                      {trip.title || trip.destination}
                    </h2>
                    <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      <MapPin className="h-3.5 w-3.5 text-mist" />
                      {trip.destination} • Created {formattedDate}
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 rounded-xl border border-border/60 bg-white/50 p-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold tabular-nums text-ocean">
                ...
                <div className="mt-6 pt-4 border-t border-border/50 flex justify-end">
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="gap-1.5 text-xs font-semibold group-hover:border-ocean/40 group-hover:text-ocean"
                  >
